...
class BloodRelationsTrainer:
    ...
    def __init__(self, args):
        ...
        Your task is to answer multiple-choice questions (MCQs) on the topic of "Blood Relations", "Seating Arrangements" and "Truth teller and liars". Apart from that you are an excellent logician and a puzzle solver
    ...
    def detect_topic(self ,questions: Dict[str, str|Any] )-> Dict:
        prompt = """
        You are an expert at solving and identifying puzzle types. Given a question , identify which of these 3 topics could the puzzle be ?
        1. Truth-teller and Liar Problems
        2. Seating Arrangements (Linear, Circular)
        3. Generations and Family tree logic
        The topic returned must strictly match one of teh above 3
        Return the response strictly in the json format
       '{{\n'
        ...
        Question : "A man's sister is the mother-in-law of my wife. My wife is the only child of her parents. How is the man's sister related to my daughter?"
        Answer : '{{\n'
            '  "topic": "Generations and Family tree logic",\n'
            '}}'
    Which category topic does this belong to ? 
        {0}

    
    """
        # print(prompt.format(questions))
        return self.agent.generate_response(prompt.format(questions), system_prompt="You are an expert at solving and identifying puzzle types")
    
        
    def get_prompt_truth(self):
        """Return the specialized prompt for truth-teller and liar problems."""
        return (
            "You are an expert in solving Truth-teller and Liar problems. "
            "These problems involve characters who either always tell the truth or always lie. "
            "Some problems may include normal people who can do either, or other variations. "
            "Use logical deduction to analyze statements and determine who is telling the truth and who is lying. "
            "Consider all possible scenarios and eliminate contradictions. "
            "Provide clear step-by-step reasoning to reach your conclusion."
        )
    
    def get_prompt_family(self):
        """Return the specialized prompt for family relationship problems."""
        return (
            "You are an expert in solving Family Relationship problems. "
            "These problems involve determining relationships between people based on given information. "
            "Draw family trees or diagrams to visualize relationships. "
            "Pay attention to terms like 'brother-in-law', 'maternal aunt', etc., which have specific meanings. "
            "Work systematically through each relationship mentioned to build a complete picture. "
            "Provide clear step-by-step reasoning to reach your conclusion."
        )
    
    def get_prompt_arrange(self):
        """Return the specialized prompt for seating arrangement problems."""
        return (
            "You are an expert in solving Seating Arrangement problems. "
            "These problems involve determining the positions of people or objects in a linear or circular arrangement. "
            "Draw diagrams to visualize the arrangement and test different possibilities. "
            "Pay attention to relative positions (left, right, next to, opposite, etc.). "
            "Systematically apply each given condition to narrow down the possibilities. "
            "Provide clear step-by-step reasoning to reach your conclusion."
        )
    ...
    def build_prompt(self, question_data: Dict[str, str|Any]) -> Tuple[str, str]:
        ...
        # Get the topic from the question data
        topic = question_data.get('topic', '')
        
        # Select the appropriate prompt based on the topic
        if topic == 'Truth-teller and Liar Problems':
            sys_prompt = self.get_prompt_truth()
        elif topic == 'Generations and Family tree logic' or topic == 'Blood Relations':
            sys_prompt = self.get_prompt_family()
        elif topic == 'Seating Arrangements (Linear, Circular)' or topic == 'Seating Arrangement':
            sys_prompt = self.get_prompt_arrange()
        else:
            sys_prompt = sys_prompt_fallback
    ...
    def _format_sft_dataset(self, raw_items: List[Dict[str, str]]) -> Dataset:
        ...
        for item in raw_items:
            ...
            if topic == 'Truth-teller and Liar Problems':
                system_prompt = self.get_prompt_truth()
            elif topic == 'Generations and Family tree logic' or topic == 'Blood Relations':
                system_prompt = self.get_prompt_family()
            elif topic == 'Seating Arrangements (Linear, Circular)' or topic == 'Seating Arrangement':
                system_prompt = self.get_prompt_arrange()
            else:
