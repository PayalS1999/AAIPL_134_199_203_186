...
class BloodRelationsTrainer:
    ...
    def generate_response(self, message: str|List[str], system_prompt: Optional[str] = None, **kwargs)->str:
        ...
        for msg in message:
            messages = [
                {"role": "system", "content": self.build_prompt(msg)},
    ...
    def build_prompt(self, question_data: Dict[str, str|Any] , topic : str):
        tmpl = f"""INSTRUCTIONS FOR ANSWERING:
            ...
            Place it between {self.reasoning_start} and {self.reasoning_end}.
            'Then, provide your answer between {self.solution_start} and {self.solution_end}
            Make sure to follow the XML format strictly i.e. start with {self.reasoning_start} and end with {self.reasoning_end} for reasoning, and start with {self.solution_start} and end with {self.solution_end} for the answer.
            Your answer should be a single letter (A, B, C, or D) representing the correct option.
            Do not include any additional text outside of these tags."""
    ...
    def _format_sft_dataset(self, raw_items: List[Dict[str, str]]) -> Dataset:
        ...
        for item in raw_items:
            ...
            # Format as chat
            chat_messages = [
                {'role': 'system', 'content': self.build_prompt(question, topic)},
