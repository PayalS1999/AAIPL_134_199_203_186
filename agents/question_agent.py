...
# Dictionary of prompt components for generating varied system prompts
PROMPT_COMPONENTS = {
    "personas": [
        "You are an expert creator of difficult logical reasoning problems for competitive exams.",
        "You are a master puzzle designer, crafting unique and challenging logic puzzles.",
        "You are a university professor writing an advanced examination on analytical reasoning.",
        "You are a data scientist designing a test to screen for top-tier logical thinking skills.",
        "You are a psychic, framing a logic puzzle as a glimpse into a complex future that must be deciphered.",
        "You are a master wordsmith, known for twisting words and phrases to create tricky questions that require careful reading.",
        "You are a professor with an exceptional vocabulary, designing a question that is as much a test of language comprehension as it is of logic."
    ],
    "difficulty_instructions": [
        "The question must be hard and require multiple steps of logical deduction to solve.",
        "The question should contain a plausible 'trap' or a distractor based on a common misconception.",
        "The question must test deep conceptual understanding, not just rote memorization of patterns.",
        "The question should be complex enough that a casual reader would likely get it wrong without a step-by-step analysis."
    ],
    "structure_requests": [
        "The explanation must be a clear, step-by-step proof of why the answer is correct.",
        "The explanation must walk through the entire logical chain, from the first premise to the final conclusion.",
        "The explanation should first state the key insight required to solve the puzzle, then provide the detailed steps."
    ]
}
...
class QuestioningAgent(object):
    ...
    def build_prompt(self, topic: str, wadvsys: bool = True, wicl: bool = True, inc_samples: List[Dict[str, str]]|None = None) -> Tuple[str, str]:
        ...
        if wadvsys:
            ...
            persona = random.choice(PROMPT_COMPONENTS["personas"])
            difficulty = random.choice(PROMPT_COMPONENTS["difficulty_instructions"])
            structure = random.choice(PROMPT_COMPONENTS["structure_requests"])
            
            # Combine the components into a cohesive system prompt
            sys_prompt = f"""
            {persona}
            {difficulty}
            {structure}
        ...
        tmpl = (
            ...
            '8.  Adhere to **Word Limits**: Ensure the total word count for the `topic`, `question`, `choices`, and `answer` fields combined is *less than 80 words. The total word count for the entire JSON object (including the explanation) must be **less than 200 words**.'
...
if __name__ == "__main__":
    ...
    inc_samples = QuestioningAgent.load_icl_samples("correct_few_shot.json") # assets/topics_example.json
    ...
    for q in question:
        try:
            ...
            q = agent.agent.generate_response(prompt.format(q), "You are an expert JSON extractor.", max_new_tokens=1024, temperature=0.5, do_sample=False) # temperature=0.0
        ques.append(q)
