...
class AnsweringAgent(object):
    ...
    def build_prompt(self, question_data: Dict[str, str|Any]) -> Tuple[str, str]:
        ...
        resp, _, _ = self.detect_topic(question_data['question'])

        try:
            try:
                clean_response = eval(resp)
                best_match = resp["topic"]
            # Strip ```json ... ``` or ``` ... ``` blocks
            except Exception:
                clean_response = re.sub(r"^```(?:json)?\n|\n```$", "", resp.strip(), flags=re.IGNORECASE | re.MULTILINE)
            ...
            # Get the best match
            best_match, score = process.extractOne(topic, topics, scorer=fuzz.token_sort_ratio)
        
        print(f"Best Match: {best_match}")
        # print(f"Score: {score}")

        # parsed = self.agent.generate_response(auto_json(topic))
        # print(parsed)
        if best_match == 'Truth-teller and Liar Problems':
            sys_prompt2 = get_prompt_truth()
        if best_match == 'Generations and Family tree logic':
            sys_prompt2 = get_prompt_family()
        if best_match == 'Seating Arrangements (Linear, Circular)':
            sys_prompt2 = get_prompt_arrange()
        else:
            sys_prompt2 = sys_prompt_fallback
    ...
    def detect_topic(self ,questions: Dict[str, str|Any] )-> Dict:
        ...
        3. Puzzles involving generations and family tree logic
